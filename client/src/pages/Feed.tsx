  ...
  if (sortedMemories.length === 0) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center pb-24 px-6">
        ...
        <h2 className="text-xl font-display font-semibold text-center mb-2">Pas encore de souvenirs</h2>
        <p className="text-muted-foreground text-center font-body">Complétez votre premier défi pour voir vos souvenirs ici !</p>
        <BottomNav />
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-foreground pb-24">
      <div className="snap-y snap-mandatory h-screen overflow-y-auto">
        {sortedMemories.map((memory) => {
          ...
          return (
            <div key={memory.id} className="snap-start h-screen relative flex items-end">
              {firstPhoto ? (
                <img src={firstPhoto} alt="" className="absolute inset-0 w-full h-full object-cover" />
              ) : (
              ...
              <div className="relative z-10 p-6 pb-28 w-full text-primary-foreground">
                <span className="text-xs bg-primary/80 px-2 py-1 rounded-full capitalize font-body">{challenge?.category}</span>
                <h2 className="text-2xl font-display font-bold mt-2 mb-1 text-card">
                  {challenge?.emoji} {challenge?.title}
                </h2>
                {memory.description && (
                  <p className="text-card/80 font-body text-sm line-clamp-3 mb-3">{memory.description}</p>
                )}
